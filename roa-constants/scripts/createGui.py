...
# Open xml
f = open(sys.argv[1] + "_config.xml")
xml = f.read()
f.close()

# Create xml tree
with  open(sys.argv[1] + '_config.xml') as f:
    tree = etree.parse(f)
...
f = open('../constants/' + sys.argv[1] + '_exports.js', 'w')
f.write(javascript)

f = open('../constants/' + sys.argv[1] + '_utils.py', 'w')
f.write(python)

f = open('../constants/' + sys.argv[1] + '_utils.cpp', 'w')
f.write(cpp)
